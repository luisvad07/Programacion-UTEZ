...
from movies.search_movies import app
from movies.search_movies.utils import get_secret, get_connection


class TestApp(unittest.TestCase):

    # Prueba: caso exitoso
    @patch("movies.search_movies.app.get_movies_by_name")
    @patch("movies.search_movies.app.get_connection")
    def test_lambda_handler_success(self, mock_get_connection, mock_get_movies_by_name):
        mock_get_connection.return_value = MagicMock()
        mock_get_movies_by_name.return_value = [
    ...
    # Prueba: falta el parámetro de búsqueda
    @patch("movies.search_movies.app.get_movies_by_name")
    @patch("movies.search_movies.app.get_connection")
    def test_lambda_handler_missing_search_param(self, mock_get_connection, mock_get_movies_by_name):
        mock_event = {
            'pathParameters': {}
        }
        ...
        body = json.loads(result['body'])
        self.assertEqual(body['message'], 'El parámetro de búsqueda es necesario')

    # Prueba: error al obtener las películas de la base de datos
    @patch("movies.search_movies.app.get_movies_by_name")
    @patch("movies.search_movies.app.get_connection")
    def test_lambda_handler_error_getting_movies(self, mock_get_connection, mock_get_movies_by_name):
        mock_get_connection.return_value = MagicMock()
        mock_get_movies_by_name.side_effect = Exception("Database query error")
    ...
    @patch("movies.search_movies.utils.boto3.session.Session.client")
    ...
    @patch("movies.search_movies.utils.get_secret")
    ...
    # Prueba: get_movies_by_name con respuesta correcta
    @patch("movies.search_movies.app.get_connection")
    def test_get_movies_by_name(self, mock_get_connection):
        ...
        result = app.get_movies_by_name('Movie')
        self.assertEqual(len(result), 2)
        self.assertEqual(result[0]['title'], 'Movie 1')
        self.assertEqual(result[1]['title'], 'Movie 2')

    # Prueba: get_movies_by_name con error de consulta
    @patch("movies.search_movies.app.get_connection")
    def test_get_movies_by_name_query_error(self, mock_get_connection):
        mock_connection = MagicMock()
        mock_connection.cursor.side_effect = Exception("Query execution error")
        mock_get_connection.return_value = mock_connection

        with self.assertRaises(Exception) as context:
            app.get_movies_by_name('Movie')
