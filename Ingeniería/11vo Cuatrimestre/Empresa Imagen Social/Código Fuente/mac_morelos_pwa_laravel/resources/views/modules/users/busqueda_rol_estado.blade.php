    ...
    <div class="flex min-h-screen">
        ...
        <div id="mainContent" class="flex-grow p-12 ml-80 transition-all duration-300">
            <div class="bg-white p-8 rounded-md w-full">
                ...
                <!-- Formulario de Búsqueda por Rol/Estado -->
                <form id="roleStatusSearchForm" class="mb-6">
                    <div class="flex space-x-4 mb-4">
                        <select name="role_status_filter" id="roleStatusFilter" class="border-gray-300 rounded-lg p-2">
                            <option value="role">Rol</option>
                            <option value="status">Estado</option>
                        </select>
                        <select name="role_status_value" id="roleStatusValue" class="border-gray-300 rounded-lg p-2 flex-grow">
                            <!-- Opciones para Rol (por ID) -->
                            <option value="1" data-filter="role">Ciudadano</option>
                            <option value="2" data-filter="role">Promotor</option>
                            <option value="3" data-filter="role">Administrador</option>
                            <option value="4" data-filter="role">Supervisor</option>
                            <option value="5" data-filter="role">Periodista</option>
                            <!-- Opciones para Estado -->
                            <option value="true" data-filter="status">Activo</option>
                            <option value="false" data-filter="status">Inactivo</option>
                        </select>
                        <button type="button" id="roleStatusSearchButton" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                            Buscar por Rol/Estado
                        </button>
                    </div>
                </form>
                ...
                <div id="roleStatusResultsTableContainer" class="hidden">
                    <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                        <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
                            <table id="roleStatusResultsTable" class="min-w-full leading-normal">
                                <thead>
                                ...
                                <tbody id="roleStatusResultsBody">
...
<script>
    ...
    $(document).ready(function() {
        // Inicializa DataTables para la tabla de Rol/Estado
        var table = $('#roleStatusResultsTable').DataTable({
            "language": {
                "url": "/js/Spanish.json"
            },
        ...
        $('#roleStatusSearchButton').on('click', function() {
            performRoleStatusSearch($('#roleStatusFilter').val(), $('#roleStatusValue').val());
        });

        function performRoleStatusSearch(filterType, searchTerm) {
            $('#loader').removeClass('hidden');
            $('#roleStatusResultsTableContainer').addClass('hidden');
            ...
            $.ajax({
                ...
                success: function(response) {
                    console.log("Respuesta de la API (Búsqueda Rol/Estado):", response);
                    displayResults(response, 'roleStatus');
                },
        ...
        function displayResults(response, tableType) {
            $('#loader').addClass('hidden');
            ...
            let resultsBodyId = (tableType === 'roleStatus') ? '#roleStatusResultsBody' : '#resultsBody';
            let tableContainerId = (tableType === 'roleStatus') ? '#roleStatusResultsTableContainer' : '#resultsTableContainer';
            let tableId = (tableType === 'roleStatus') ? '#roleStatusResultsTable' : '#resultsTable';
            ...
            if (results.length > 0) {
                ...
                let tableData = results.map(user => {
                    ...
                    return [
                        ...
                        `<span class="${statusClass}">${statusText}</span>`, // Quinta columna: Estado
                        `<button class="text-gray-600 hover:text-gray-900 view-user" data-user='${JSON.stringify(user)}'><i class="material-icons">visibility</i></button>` // Sexta columna: Botón "Ver"
                    ];
                });
        ...
        // Manejar la visibilidad de las opciones del segundo select
        $('#roleStatusFilter').change(function() {
            const selectedFilter = $(this).val();

            $('#roleStatusValue option').each(function() {
        ...
        }).trigger('change'); // Llama al evento change al cargar la página para configurar la visibilidad inicial
    });
