...
import static server.Method.mostrarRegistro;

public class clientCURP {
    static Scanner teclado = new Scanner(System.in);
    static BeanCURP beanCURP;
            ...
            choix = teclado.next();
                switch (Integer.parseInt(choix)) {
                    case 1:
                        ...
                        String CURP = Method.generarCurp(apellido_Pat,apellido_Mat,nombre, sexo,fecha_Nac,estado_Nac);
                        System.out.println(CURP);
                        data = new Object[]{nombre, apellido_Pat, apellido_Mat, sexo, estado_Nac, fecha, CURP};
                        response = (String) client.execute("Method.savePerson", data);
                        System.out.println("Result -> "+response);
                        break;
                    case 2:
                        System.out.println("CONSULTA DE DATOS");
                        System.out.print("Ingresa la CURP a buscar: ");
                        String busq = teclado.next();
                        if (busq.equals(beanCURP.getCURP())){
                            Method.mostrarRegistro(busq);
                        } else {
