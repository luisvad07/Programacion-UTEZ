...
export const DepRep = () => {
    ...
    const url = `http://${localhost}:8080/redre/estudiante/departamento/${sessionId}/hasReport`
    ...
    const aprobar = async () => {
        ...
        axios.put(`http://${localhost}:8080/redre/mensajesDepartamento/aprobado/${estudianteMatricula}`)
        .then((response) => {
            console.log(response);
            let isTSU = (grado===6);
            const f = new FormData();
                f.append("matricula", estudianteMatricula);
                f.append("division", localStorage.getItem("divisionAcademica"));
                f.append("type", isTSU ? "TSU" : "ING");
            axios.post(`http://${localhost}:8080/redre/files/uploadFinal`, f).then((response) => {
                console.log(response);
    ...
    const rechazarReporte = () => {
        ...
        parametros = {
            ...
            nombreD: localStorage.getItem('nombre')
        }
        enviarMensaje(parametros)
    }

    const enviarMensaje = async (parametros)  => {
        const url = `http://${localhost}:8080/redre/mensajesDepartamento/Dp/`
    ...
    const handleShow = (modal, matricula, grado) => {
        if (modal === 'validacion') {
                                ...
                                {estudiante.length === 0 ? (
                                        ...
                                        estudiante.map((estudianten, index) => (
                                            ...
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <Button style={{ background: '#3D9982' }} onClick={() => handleShow('validacion', estudianten.matricula, estudianten.grado)}>
                                                        <FeatherIcon icon={"check-circle"} style={{ stroke: "white" }} />
                                                    </Button>
                                                </td>
                                                <td>
                                                    <Button className='btn-danger' onClick={() => handleShow('retroalimentacion', estudianten.matricula, estudianten.grado)}>
