...
public class DaoAuth {
    ...
    private final String GET_LOGIN = "SELECT * FROM person JOIN user ON person.id = user.person WHERE username = ? AND password = ?";
    ...
    public BeanAuth login(String username, String password){
        ...
        try {
            ...
            Logger.getLogger(DaoAuth.class.getName()).log(Level.SEVERE,"Error en DaoAuth.login->"+e);
            return null;
        } finally {
            closeConnections();
        }
