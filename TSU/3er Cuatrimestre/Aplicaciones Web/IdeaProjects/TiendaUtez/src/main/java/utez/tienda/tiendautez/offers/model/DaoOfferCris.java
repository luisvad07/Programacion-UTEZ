...
public class DaoOfferCris {
    ...
    public void checkDateOffer(){
        ...
        for (BeanOffer beanOffer : offerList2) {
            //System.out.println(beanOffer.getId_offers());
            // System.out.println(beanOffer.getEnd_date2());
            // System.out.println(fa);
            id = Math.toIntExact(beanOffer.getId_offers());
            int star = beanOffer.getStart_date2().compareToIgnoreCase(fa);
            if (beanOffer.getEnd_date2().equalsIgnoreCase(fa) || star ==1){

                //System.out.println("No vigente");
                boolean al =  daoOfferCris.updateStatus(id , 1);
                List<ProductUserBean> productsOffers = new ProductUserDao().status_objets(id);

                for (ProductUserBean products: productsOffers) {
                    //System.out.println(products.getId_products());
                    int id_product = products.getId_products();
                    boolean FK_offer_products = new ProductUserDao().updateStatusProduct(id_product);
                }


            }else {
                boolean al =  daoOfferCris.updateStatus(id , 0);
            }
        }
    }
